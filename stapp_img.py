...
def main():
    ...
    if uploaded_file is not None:
        ...
        pred0 = round(prediction[0],3) * 100  # きよし師匠の確率(%)
        pred1 = round(prediction[1],3) * 100  # 和田アキ子の確率(%)
        pred2 = round(prediction[2],3) * 100  # あき竹城の確率(%)

        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「きよし師匠」です！")
        elif classNo == 1:
            st.subheader(f"これは{pred1}％の確率で「和田アキ子」です！")
        else :
            st.subheader(f"これは{pred2}％の確率で「あき竹城」です！")
